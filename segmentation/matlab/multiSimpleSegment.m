function [segmentSeries tmpOn durations] = multiSimpleSegment(tmpSeries, nbSegments)
% Perform piecewise linear approximation on the series
segments = simpleSegment(tmpSeries, nbSegments, 1);
...
while ~(curSegment.lx == 1 && curSegment.rx == length(tmpSeries))
    ...
    for i = 1:(length(tmpQueue) - 1)
        ...
        deltaEntropy(i) = tmpEntropy - (tmpQueue(i).entropy + tmpQueue(i + 1).entropy);
    end
...
% assignin('base', 'newQueue', newQueue)
% assignin('base', 'segmentSeries', segmentSeries)
% assignin('base', 'tmpOn', tmpOn)
